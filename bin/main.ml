...
let () =
  ...
  if input_files = [] then (
    Printf.eprintf
      "Usage: %s <filename>... [-o <output.o>] [--target <triple>] [--cpu <cpu>] [--features <features>] [-g] [--profile-functions] [--forbid-trap] [--forbid-unsafe] [--emit-exception-frame-offsets <StructName>] [--version]\n"
      Sys.argv.(0);
    exit 1
  );
