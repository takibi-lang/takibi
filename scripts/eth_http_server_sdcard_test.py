...
def fetch_with_retry(path: str) -> tuple:
    """The fetch, waiting out a link this board has not finished negotiating.

    This used to retry on ANY OSError, which is the unsound version of the
    same idea: a timeout or a refused connection can mean the request
    ARRIVED and its answer was lost, so retrying through one asks the server
    to do the work twice. Nothing here counts requests today, so it did not
    produce a wrong verdict -- but it is the shape that does, and the
    distinction now lives in one place (GitHub issue #387).

    Narrowing matches this script's own recorded evidence: the failure it
    was written for is "No route to host" after the second reset, which is
    EHOSTUNREACH, generated by the host before anything reaches the wire.

    The ARP flush stays inside the attempt, so every try still forces a cold
    resolution rather than reusing an entry the previous try created.
    """
    def attempt():
        flush_arp_entry()
        return fetch(path)
    ...
    return wait_until_reachable(attempt, seconds=RETRY_TOTAL_SECS,
                                poll=RETRY_INTERVAL_SECS, label=path)
