...
def main() -> int:
    ...
    if failures:
        ...
        return 1
    report_pass(
        "irq-restore controls",
        "the repository passes, issue #454's own defect and a guard "
        "dropped from a restore that had one are each refused, a "
        "declaration that outlives its site is refused, and the "
