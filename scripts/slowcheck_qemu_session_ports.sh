...
[ "$(offset_for "$registry")" = 0 ] \
    && claim || fail "a session with no name must take no offset"
[ "$(offset_for "$tmp_dir/absent" claude)" = 0 ] \
    && claim || fail "a missing registry directory must take no offset"

# Blocks are handed out in order, and a name keeps the block it was given.
[ "$(offset_for "$registry" takibi-claude)" = 0 ] && claim || fail "first block is not 0"
[ "$(offset_for "$registry" takibi-codex)" = "$stride" ] && claim || fail "second block is not 1"
[ "$(offset_for "$registry" takibi-codex-3)" = "$((stride * 2))" ] \
    && claim || fail "a name outside any fixed list must still get a block"
[ "$(offset_for "$registry" takibi-claude)" = 0 ] \
    && claim || fail "a session's block is not stable across calls"
[ "$(offset_for "$registry" takibi-claude 7000)" = 7000 ] \
    && claim || fail "an explicit offset must win over the registry"
...
[ "$lines" = "$blocks" ] && [ "$distinct_blocks" = "$blocks" ] \
    ...
    && claim || fail "$blocks concurrent claims produced $lines lines, $distinct_blocks blocks"
...
timeout 5 env TAKIBI_SESSION_REGISTRY="$killed" TAKIBI_SESSION=after-kill \
    ...
    && claim || fail "a killed holder left the registry lock stuck"
...
grep -F "port blocks are claimed" "$tmp_dir/overflow.log" >/dev/null \
    && claim || fail "exhaustion did not say the registry is full"
...
grep -F "$broken/blocks" "$tmp_dir/broken.log" >/dev/null \
    && claim || fail "the malformed-registry message does not name the file"
...
[ "$shifted" = "$((18673 + stride)) $((18080 + stride)) 18671" ] \
    && claim || fail "shifting named ports produced '$shifted'"
...
grep -F "outside this session's repeat window" "$tmp_dir/repeat-count.log" \
    >/dev/null && claim || fail "an oversized repeat run did not name the window"
...
bash "$repeat" --mode check "$QEMU_SESSION_REPEAT_MAX_SAMPLES" true \
    >/dev/null 2>&1 && claim || fail "the largest fitting repeat run was refused"
...
echo "PASS qemu-session-ports: $cases claims -- blocks are stable, exclusive, bounded, and survive a killed holder"
