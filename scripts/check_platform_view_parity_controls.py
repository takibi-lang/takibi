...
def main() -> int:
    ...
    if failures:
        ...
        return 1
    report_pass(
        "platform-view-parity controls",
        "the repository passes, an undeclared platform view and two "
        "copies of one expected file are each refused, a declaration "
        "that outlives its view is refused, and a declared one is "
        "accepted",
        cases=CASES.ran)
    return 0
