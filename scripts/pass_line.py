...
total, the way `scripts/buildcheck_kernel_memory_map.py` counts state-tagged rows
...
class CaseCount:
    """How many scenarios a control actually ran.

    A control's PASS says its scenarios all behaved, and that claim is empty
    if the scenarios did not run. GitHub issue #526 found controls to be the
    one group in this directory nothing held to that rule -- `check_*.py` had
    to assert a count and their controls did not, which is the exact vacuity
    `report_pass` exists to refuse, one level up.

    So a control counts where it runs and asserts the count where it reports.
    Note it inside the helper that runs one scenario; a loop that never
    executes then reports zero, and `report_pass` refuses zero.
    """
...
def report_pass(check: str, message: str, *, stream=None,
    ...
    UART report in `buildcheck_suite_output.py` is read field by field by
