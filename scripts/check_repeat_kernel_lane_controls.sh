...
[ "$status" -eq 1 ] && claim || fail "check mode exited $status, expected 1"
grep -q "sample 1 of 5" "$out" && claim || fail "check mode did not stop at the first sample"
[ -d "$tmp_dir/check/sample-1" ] && claim || fail "check mode kept no artifacts for the failing sample"
...
[ "$status" -eq 0 ] && claim || fail "check mode failed on a command that always succeeds"
grep -q "PASS repeat/ctl-ok: 3 independent runs" "$tmp_dir/ok.log" && claim ||
...
grep -q "6 runs -> 4 pass, 2 fail" "$tmp_dir/rate.log" && claim ||
    fail "measure mode did not run every sample: $(grep 'runs ->' "$tmp_dir/rate.log" || true)"
grep -q "observed rate 2/6" "$tmp_dir/rate.log" && claim || fail "measure mode reported no rate"
grep -q "consecutive clean runs needed" "$tmp_dir/rate.log" && claim ||
    fail "measure mode reported no confidence requirement"
[ -d "$tmp_dir/rate/sample-6" ] && claim || fail "measure mode kept no artifacts for the last sample"
...
grep -q "no failures observed" "$tmp_dir/clean.log" && claim || fail "clean run reported nothing"
grep -q "if the real rate were 1 in" "$tmp_dir/clean.log" && claim ||
...
grep -q "3 runs -> 0 pass, 3 fail" "$tmp_dir/all.log" && claim ||
    fail "an all-failing measurement did not report its counts"
grep -q "not intermittent at this load" "$tmp_dir/all.log" && claim ||
...
echo "PASS repeat-runner-control: $cases claims -- check stops at the first failure, measure reports the rate, and an all-failing run reports rather than raises"
