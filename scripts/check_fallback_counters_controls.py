...
def main() -> int:
    ...
    if failures:
        ...
        return 1
    report_pass(
        "fallback-counters controls",
        "the repository passes, a counter with no reader, an advisory "
        "report, an unexpected positive line, a missing gate, and a "
        "wholesale rename are each refused",
        cases=CASES.ran)
    return 0
