...
[ "$(sed -n 1p "$log.normalized")" = "alpha: one" ] ||
    fail "the interactive prompt was not stripped"
[ "$(sed -n 3p "$log.normalized")" = "[<time>] gamma: three" ] ||
    fail "the dmesg timestamp prefix was not normalized"
[ "$(sed -n 4p "$log.normalized")" = "persistent shell: child forked pid=<child>" ] ||
...
kernel_views_compare control "$artifacts" "$log.normalized" \
    "$common" "$platform" >/dev/null 2>&1 ||
    fail "a tree whose every view matches was refused"
[ "$kernel_views_passed" -eq 3 ] ||
    fail "counted $kernel_views_passed views, expected 3"
[ -z "$kernel_views_failed" ] || fail "a passing run named failures"
...
[ "$status" -eq 1 ] || fail "two failing views returned $status, expected 1"
[ "$kernel_views_failed" = " first second" ] ||
    fail "named '$kernel_views_failed', expected both failing views"
grep -q "^PASS control view: third$" "$report" ||
...
[ ! -f "$artifacts/removed_view.actual" ] ||
...
kernel_views_compare control "$artifacts" "$log.normalized" \
    "$common" "$platform" "$overlay" >/dev/null 2>&1 ||
...
[ "$status" -eq 2 ] || fail "an empty view set returned $status, expected 2"
...
[ "$status" -eq 2 ] || fail "a filter with no expected file returned $status, expected 2"

echo "PASS kernel-views controls: normalization strips the prompt, the CR, the dmesg timestamp and the minted pid; every mismatch is named rather than the first; a stale .actual is purged; platform and overlay lookup win in that order; and comparing nothing fails"
