...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as name:
        tree = Path(name)
        for label, body, expected in SCENARIOS:
    ...
    report_pass(
        "pipefail-early-exit controls",
        "the pipeline that broke CI and four other early-exit consumers "
        "are found, reading to EOF and a small producer and a script "
        "without pipefail are not, and editing a declared line brings "
        "it back",
        cases=CASES.ran)
    return 0
