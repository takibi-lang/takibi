...
Nothing at EL0 can tell where a syscall ran: /bin/affinity's rt_sigprocmask answers
the same either way. So this watches the kernel instead, without the kernel
printing anything for it.

The probe pins itself to CPU 1 and asks for rt_sigprocmask, which is outside the
...
PINNED = b"affinity: pinned to cpu 1, where rt_sigprocmask, outside the peer-safe table, still answered"
...
def run() -> None:
    ...
    if asked.hit_count == 0:
        verdict(False, "/bin/affinity never asked rt_sigprocmask from CPU 1, so "
    ...
    continue_bounded()
    if gate.hit_count == 0:
        verdict(False, "/bin/affinity asked rt_sigprocmask from CPU 1 and the "
    ...
    if rerun.hit_count != 1:
        verdict(False, "the gate handed rt_sigprocmask off on CPU1, but core 0 never "
    ...
    if not seen(lambda text: PINNED in text, STEP_TIMEOUT):
        ...
        gdb.execute(f"target remote 127.0.0.1:{GDB_PORT}")
        verdict(False, "core 0 reran rt_sigprocmask, but /bin/affinity never printed "
                f"its pinned line. {where()}. "
                f"UART tail: {uart_tail()!r}")
        gdb.execute("detach")
        return
    verdict(True, "rt_sigprocmask asked from CPU1 took the migration gate there, "
