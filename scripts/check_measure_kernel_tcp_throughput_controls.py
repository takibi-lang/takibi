...
def main() -> int:
    ...
    report_pass(
        "tcp-throughput controls",
        "a healthy transfer reports its rate and a spread, zero bytes "
        "and zero seconds are refused as measuring nothing, transfers "
        "of differing size are not averaged, a refused connection is "
        "reported, and the artifact records bytes, commit and per-run "
        "figures",
        cases=CASES.ran)
    return 0
