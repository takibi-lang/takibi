...
[ -n "$one" ] && claim || fail "no archive directory was created"
[ -f "$one/uart.log" ] && claim || fail "uart.log was not kept"
[ -f "$one/nested/extra.log" ] && claim || fail "nested files were not kept"
...
[ "$count" -eq 2 ] && claim || fail "two archives collapsed into $count directory(ies)"
