...
reason="$(board_link_gate "$log" "$READY" "$FAILED" 100)" \
    || fail "a log that already holds the ready line was not accepted"
grep -q "^link ready " <<<"$reason" \
    || fail "a ready link did not report how long it took: $reason"
...
writer=$!
reason="$(board_link_gate "$log" "$READY" "$FAILED" 100)" \
    || fail "a link that arrived after 0.6s was not waited for"
wait "$writer"
grep -q "^link ready " <<<"$reason" || fail "late ready reported oddly: $reason"
...
[ $((SECONDS - started)) -le 5 ] \
    || fail "a reported failure spent the whole window being discovered"
grep -q "the target reported" <<<"$reason" \
    || fail "a reported failure was not named: $reason"
...
grep -q "never printed" <<<"$reason" || fail "a silent target: $reason"
grep -q "NOT a defect in the tests that follow" <<<"$reason" \
    || fail "a silent target's reason does not say the tests below are not at fault: $reason"
...
grep -q "never printed" <<<"$reason" || fail "a missing log: $reason"

echo "PASS board-link-gate controls: a ready link reports its wait, a late "\
