...
if [ "$current_mode" = "EL2H" ] && [ "$halted_pc" = "0x0000000000080000" ]; then
    echo "reset confirmed: back in examples/common_rpi5/jtag_stub.S's spin loop (PC=$halted_pc)"
else
    echo "warning: reset halted, but PC=$halted_pc mode=$current_mode does not look like" \
         "the spin stub -- either the reset did not reboot the SoC (in which case this" \
         "just halted whatever was already running), or kernel_2712.img on the SD card" \
