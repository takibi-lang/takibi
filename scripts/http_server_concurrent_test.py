...
# MAX_CONNS=8 slot table: independent admission, independent per-slot
...
def test_table_full_ninth_syn_silently_ignored() -> bool:
    """Fill all MAX_CONNS=8 slots with un-closed connections, then confirm
    a 9th SYN gets no reply at all (the existing "unknown peer, ignore"
    ...
    try:
        ...
        try:
            ...
            print("  unexpected reply to 9th SYN:", stray.hex())
            ok = False
        except socket.timeout:
            pass
...
def main() -> int:
    ...
    ok3 = test_table_full_ninth_syn_silently_ignored()
    print("  table full: 9th SYN silently ignored:          %s" % ("PASS" if ok3 else "FAIL"))
