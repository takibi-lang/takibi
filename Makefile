...
$(KERNEL_EXT2_IMAGE): $(KERNEL_EXT2_FIXTURE_DIR)/hello.txt $(KERNEL_EXT2_FIXTURE_DIR)/mutable.txt $(KERNEL_EXT2_FIXTURE_DIR)/index.html $(KERNEL_EXT2_FIXTURE_DIR)/init.sh $(KERNEL_EXT2_FIXTURE_DIR)/large.txt $(KERNEL_RPI5_USER_PAYLOAD_ELF) $(KERNEL_RPI5_CLONE_CHAIN_ELF) | $(KERNEL_USER_BUILD_DIR)
	rm -f $@.tmp
...
# GitHub issue #246: the closing-bar demonstration payload -- same shape as
# KERNEL_RPI5_USER_PAYLOAD_ELF above (standalone static-PIE ELF, no
# writable globals, loaded via the general-purpose ELF loader), but no
