...
KERNEL_ASM_ENTRIES := main kernel_secondary_main \
	...
	kernel_core0_idle_tick_reenter kernel_secondary_idle_tick_reenter \
...
# GitHub issue #339: the kernel's own USB bring-up stage report and its retry
# rule, compiled and run natively. Same source usb_xhci.tkb links -- the
# stage names and "keep the first failing status" are byte work, and the
...
KERNEL_EXT2_MUTATION_LOCK_TKB := $(KERNEL_DIR)/fs/ext2/mutation_lock.tkb
KERNEL_EXT2_MUTATION_PROBE_TKB := $(KERNEL_DIR)/kernel/ext2_mutation_contention_evidence.tkb
...
KERNEL_BUSY_LOOP_PLACEMENT_REPORT_ELF := $(KERNEL_BUILD_DIR)/busy_placement_report.elf
KERNEL_BUSY_LOOP_PLACEMENT_GUARD_ELF := $(KERNEL_BUILD_DIR)/busy_placement_guard.elf
...
KERNEL_AFFINITY_TKB      := $(KERNEL_DIR)/arch/arm64/kernel/affinity_probe.tkb
KERNEL_AFFINITY_O        := $(KERNEL_BUILD_DIR)/affinity_probe.o
KERNEL_AFFINITY_ELF      := $(KERNEL_BUILD_DIR)/affinity.elf
KERNEL_SPREAD_TKB        := $(KERNEL_DIR)/arch/arm64/kernel/spread_probe.tkb
KERNEL_SPREAD_O          := $(KERNEL_BUILD_DIR)/spread_probe.o
KERNEL_SPREAD_ELF        := $(KERNEL_BUILD_DIR)/spread.elf
KERNEL_MOVECOST_TKB      := $(KERNEL_DIR)/arch/arm64/kernel/movecost_probe.tkb
KERNEL_MOVECOST_O        := $(KERNEL_BUILD_DIR)/movecost_probe.o
KERNEL_MOVECOST_ELF      := $(KERNEL_BUILD_DIR)/movecost.elf
KERNEL_PEER_MUTATE_TKB   := $(KERNEL_DIR)/arch/arm64/kernel/peer_mutate_probe.tkb
KERNEL_PEER_MUTATE_O     := $(KERNEL_BUILD_DIR)/peer_mutate_probe.o
KERNEL_PEER_MUTATE_ELF   := $(KERNEL_BUILD_DIR)/peer_mutate.elf
...
$(KERNEL_EXT2_IMAGE): Makefile $(KERNEL_EXT2_FIXTURE_DIR)/hello.txt $(KERNEL_EXT2_FIXTURE_DIR)/mutable.txt $(KERNEL_EXT2_FIXTURE_DIR)/index.html $(KERNEL_EXT2_FIXTURE_DIR)/about.html $(KERNEL_EXT2_FIXTURE_DIR)/icon.png $(KERNEL_EXT2_FIXTURE_DIR)/init.sh $(KERNEL_EXT2_FIXTURE_DIR)/httpd.sh $(KERNEL_EXT2_FIXTURE_DIR)/script-shebang.sh $(KERNEL_EXT2_FIXTURE_DIR)/script-interpreter-argument.sh $(KERNEL_EXT2_FIXTURE_DIR)/not-a-program $(KERNEL_EXT2_FIXTURE_DIR)/bad-interpreter.sh $(KERNEL_EXT2_FIXTURE_DIR)/crlf.sh $(KERNEL_EXT2_FIXTURE_DIR)/no-newline.sh $(KERNEL_EXT2_FIXTURE_DIR)/long-shebang.sh $(KERNEL_EXT2_FIXTURE_DIR)/inittab $(KERNEL_EXT2_FIXTURE_DIR)/large.txt $(KERNEL_RPI5_USER_PAYLOAD_ELF) $(KERNEL_BUSY_LOOP_A_ELF) $(KERNEL_BUSY_LOOP_B_ELF) $(KERNEL_BUSY_LOOP_PLACEMENT_REPORT_ELF) $(KERNEL_BUSY_LOOP_PLACEMENT_GUARD_ELF) $(KERNEL_BUSY_LOOP_PEER_SPIN_ELF) $(KERNEL_BUSY_LOOP_PEER_NET_WAKE_ELF) $(KERNEL_PEER_READ_ELF) $(KERNEL_CORE_READ_ELF) $(KERNEL_PEER_CONSOLE_ELF) $(KERNEL_PEER_TTY_ELF) $(KERNEL_CLOEXEC_ELF) $(KERNEL_CLOEXEC_CHECK_ELF) $(KERNEL_PPOLL_PROBE_ELF) $(KERNEL_AFFINITY_ELF) $(KERNEL_SPREAD_ELF) $(KERNEL_MOVECOST_ELF) $(KERNEL_PEER_MUTATE_ELF) $(KERNEL_BUSYBOX_STATIC) $(KERNEL_BUSYBOX_EXTRAS) $(KERNEL_MUSL_LOADER) $(KERNEL_TASKSET) scripts/make_interp_probe_elf.py | $(KERNEL_USER_BUILD_DIR)
	...
	dd if=/dev/zero bs=98304 count=1 status=none | tr '\0' Z >$@.peer_read.tmp
	E2FSPROGS_FAKE_TIME=1700000000 e2cp $@.peer_read.tmp $@.tmp:/peer-read.bin
	rm -f $@.peer_read.tmp
	...
	E2FSPROGS_FAKE_TIME=1700000000 e2cp $(KERNEL_BUSY_LOOP_PLACEMENT_REPORT_ELF) $@.tmp:/etc/placement-report
	E2FSPROGS_FAKE_TIME=1700000000 e2cp $(KERNEL_BUSY_LOOP_PLACEMENT_GUARD_ELF) $@.tmp:/etc/placement-guard
	E2FSPROGS_FAKE_TIME=1700000000 e2cp $(KERNEL_BUSY_LOOP_PEER_SPIN_ELF) $@.tmp:/bin/peer-spin
	E2FSPROGS_FAKE_TIME=1700000000 e2cp $(KERNEL_BUSY_LOOP_PEER_NET_WAKE_ELF) $@.tmp:/bin/peer-net-wake
	...
	E2FSPROGS_FAKE_TIME=1700000000 e2cp $(KERNEL_AFFINITY_ELF) $@.tmp:/bin/affinity
	E2FSPROGS_FAKE_TIME=1700000000 e2cp $(KERNEL_SPREAD_ELF) $@.tmp:/bin/spread
	...
	debugfs -w -R 'set_inode_field /etc/placement-report mode 0100755' $@.tmp >/dev/null 2>&1
	debugfs -w -R 'set_inode_field /etc/placement-guard mode 0100755' $@.tmp >/dev/null 2>&1
	debugfs -w -R 'set_inode_field /bin/peer-spin mode 0100755' $@.tmp >/dev/null 2>&1
	debugfs -w -R 'set_inode_field /bin/peer-net-wake mode 0100755' $@.tmp >/dev/null 2>&1
	...
	debugfs -w -R 'set_inode_field /bin/affinity mode 0100755' $@.tmp >/dev/null 2>&1
	debugfs -w -R 'set_inode_field /bin/spread mode 0100755' $@.tmp >/dev/null 2>&1
	...
	debugfs -w -R 'link /bin/busybox.static /bin/sleep' $@.tmp >/dev/null 2>&1
	...
	debugfs -w -R 'set_inode_field /bin/busybox.static links_count 37' $@.tmp >/dev/null 2>&1
...
$(KERNEL_BUSY_LOOP_PLACEMENT_REPORT_ELF): $(KERNEL_BUSY_LOOP_O)
	$(LLD) -pie --no-dynamic-linker -e busy_loop_placement_report $< -o $@
	python3 scripts/buildcheck_user_payload_no_rw_globals.py $@

$(KERNEL_BUSY_LOOP_PLACEMENT_GUARD_ELF): $(KERNEL_BUSY_LOOP_O)
	$(LLD) -pie --no-dynamic-linker -e busy_loop_placement_guard $< -o $@
	python3 scripts/buildcheck_user_payload_no_rw_globals.py $@
...
$(KERNEL_BUSY_LOOP_PEER_NET_WAKE_ELF): $(KERNEL_BUSY_LOOP_O)
...
$(KERNEL_PEER_READ_ELF): $(KERNEL_PEER_READ_O)
	$(LLD) -pie --no-dynamic-linker -e peer_read $< -o $@
	python3 scripts/buildcheck_user_payload_no_rw_globals.py $@

$(KERNEL_CORE_READ_ELF): $(KERNEL_PEER_READ_O)
	$(LLD) -pie --no-dynamic-linker -e core_read $< -o $@
	python3 scripts/buildcheck_user_payload_no_rw_globals.py $@
...
$(KERNEL_CLOEXEC_ELF): $(KERNEL_CLOEXEC_O)
	$(LLD) -pie --no-dynamic-linker -e cloexec_probe $< -o $@
	python3 scripts/buildcheck_user_payload_no_rw_globals.py $@
...
$(KERNEL_AFFINITY_ELF): $(KERNEL_AFFINITY_O)
...
$(KERNEL_SPREAD_ELF): $(KERNEL_SPREAD_O)
...
$(KERNEL_MOVECOST_ELF): $(KERNEL_MOVECOST_O)
...
$(KERNEL_PEER_MUTATE_ELF): $(KERNEL_PEER_MUTATE_O)
...
$(KERNEL_RPI5_DEBUG_ELF): $(KERNEL_RPI5_ENTRY_O) $(KERNEL_RPI5_USER_ENTRY_O) $(KERNEL_RPI5_FPSIMD_O) $(KERNEL_RPI5_PMU_O) $(KERNEL_RPI5_MAIN_DEBUG_O) $(KERNEL_RPI5_LINK_LD) $(KERNEL_DIR)/arch/arm64/boot/link_qemu.ld
...
$(KERNEL_QEMU_ELF): $(KERNEL_QEMU_ENTRY_O) $(KERNEL_QEMU_USER_ENTRY_O) $(KERNEL_QEMU_FPSIMD_O) $(KERNEL_QEMU_PMU_O) $(KERNEL_QEMU_MAIN_O) $(KERNEL_QEMU_LINK_LD) $(KERNEL_RPI5_LINK_LD)
...
$(KERNEL_QEMU_DEBUG_ELF): $(KERNEL_QEMU_ENTRY_O) $(KERNEL_QEMU_USER_ENTRY_O) $(KERNEL_QEMU_FPSIMD_O) $(KERNEL_QEMU_PMU_O) $(KERNEL_QEMU_MAIN_DEBUG_O) $(KERNEL_QEMU_LINK_LD) $(KERNEL_RPI5_LINK_LD)
...
kernelcheck-rpi5: kernelbuild-check
	@bash scripts/run_lane.sh $@ $(MAKE) _kernelcheck-rpi5
...
kernelcheck-ddb-rpi5-software: kernelbuild-check
	@bash scripts/run_lane.sh $@ $(MAKE) _kernelcheck-ddb-rpi5-software

_kernelcheck-ddb-rpi5-software:
...
kernelcheck-qemu-fdt-multibank: kernelbuild-check
	@bash scripts/run_lane.sh $@ $(MAKE) _kernelcheck-qemu-fdt-multibank

_kernelcheck-qemu-fdt-multibank:
	@python3 kernel/tests/check_fdt_multibank_qemu.py

kernelcheck-qemu-ash: kernelbuild-check
	@bash scripts/run_lane.sh $@ $(MAKE) _kernelcheck-qemu-ash

_kernelcheck-qemu-ash:
...
kernelcheck-oops-qemu: kernelbuild-check
	@bash scripts/run_lane.sh $@ $(MAKE) _kernelcheck-oops-qemu
...
kernelcheck-ddb-qemu: kernelbuild-check
	@bash scripts/run_lane.sh $@ $(MAKE) _kernelcheck-ddb-qemu
...
kernelcheck-stack-overflow-qemu: kernelbuild-check
	@bash scripts/run_lane.sh $@ $(MAKE) _kernelcheck-stack-overflow-qemu
