...
$(KERNEL_PEER_READ_O): $(KERNEL_PEER_READ_TKB) $(TAKIBI) | $(KERNEL_BUILD_DIR)
	$(TAKIBI) $< --target $(RPI5_TARGET) --cpu $(RPI5_CPU) --forbid-trap --reject-unused-functions --external-entry peer_read --external-entry core_read --emit-depfile $@.d -o $@
