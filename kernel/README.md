...
PASS kernel/rpi5 (45 views, one boot)
```
...
- `scripts/kernel_debug_metadata.gdb`: load
  ...
  compiler-owned discriminants and target layout. At a function-entry stop,
  `takibi-force-variant-return TYPE CASE` can force a payload-free case using
  the sidecar's target return ABI. The sidecar is generated by the compiler
...
- **Boot model differs from RPi5's TF-A hand-off.** QEMU `virt` (with a
  ...
  Both platforms decode the mandatory PSCI `method` from the boot DTB and
  route CPU_ON through the selected compiler intrinsic; the QEMU contract
  requires `hvc`, while the RPi5 contract requires `smc`, and either platform
  halts before SMP bring-up if the enabled node is missing or disagrees.
