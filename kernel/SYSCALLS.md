...
| 64 | write | Implemented | same three paths as `read`, plus the inetd-response path. A write to the UART terminal is copied in chunks into the console transmit queue while the queue is at or below half full, and returns a short count when it fills; one that can queue nothing sleeps until the TX interrupt has drained the queue back to half, rather than spinning in the kernel, or waits for that interrupt when nothing else can run |
