...
| 25 | fcntl | Partial | `F_DUPFD` and `F_DUPFD_CLOEXEC` (the lowest free descriptor at or above the argument, sharing the open file), and `F_SETFD` with 0 or `FD_CLOEXEC`; `execve` then closes every descriptor so marked. `F_GETFD`, `F_GETFL`/`F_SETFL` and locking return `ENOSYS`, and a descriptor that is not open returns `EBADF` |
| 34 | mkdirat | Implemented | creates an ext2 directory with `.`, `..`, the parent's link count and the group's directory count, from an absolute path, a path relative to an ext2 directory FD, or `AT_FDCWD`; trailing slashes name the same entry. `EEXIST` for an existing name, `.` or `..`; `ENOENT` for a missing parent; `ENOTDIR` when the parent is not a directory; `EIO` where Linux would say `ENOSPC`, because the ext2 layer does not yet tell a full directory or volume from a malformed one. The mode is not applied: every directory is 0755 and every process is root. Reached by BusyBox `mkdir` from the shared ash script on both lanes |
| 35 | unlinkat | Partial | with no flag (unlink): removes a regular file's entry; a file with more than one link loses one link, and the last link releases its data blocks and inode. `ENOENT`, `ENOTDIR` for a non-directory parent, `EISDIR` for a directory, `.` or `..`, and `EPERM` for a symlink or a file reaching an indirect block, which the ext2 layer does not release. With `AT_REMOVEDIR` (rmdir): removes a one-block ext2 directory holding only `.` and `..`, returning the parent's link count and the group's directory count; `ENOTEMPTY` for a non-empty directory or `..`, `EINVAL` for `.`, `ENOENT`, `ENOTDIR`, and `EIO` for a directory that grew past one block. Any other flag is `EINVAL`. Reached by BusyBox `rm` and `rmdir` from the shared ash script on both lanes |
| 38 | renameat | Partial | renames a regular file within a directory or into another one, and a directory within its own parent; an existing regular-file target is replaced, as `rename(2)` specifies, and a target that already names the source is a successful no-op. `ENOENT`, `ENOTDIR` for a non-directory parent or a directory renamed over a file, `EISDIR` for a file renamed over a directory, `EBUSY` for `.` or `..`, and `EPERM` for a directory moved to another parent, which the ext2 layer does not do. Reached by BusyBox `mv` from the shared ash script on both lanes |
...
| 63 | read | Implemented | connected-socket, ext2-file, bounded procfs-file, and UART-fallback paths, all through the typed user-memory boundary (`kernel/mm/user_memory.tkb`); an empty UART RX path blocks and wakes on received input, as exercised by the foreground interactive ash REPL. Received bytes no reader has taken wait in a 4096-byte ring, Linux n_tty's size (`kernel/drivers/serial/uart_rx_ring.tkb`); a byte that arrives when it is full is counted, and the boot's total is printed as `uart rx: capacity=4096 dropped=N` |
| 64 | write | Implemented | same three paths as `read`, plus the inetd-response path. A write to the UART terminal is copied in chunks into the console transmit queue while the queue is at or below half full, and returns a short count when it fills; one that can queue nothing sleeps until the TX interrupt has drained the queue back to half, rather than spinning in the kernel, or waits for that interrupt when nothing else can run. On a peer CPU the bytes go to core 0 as 64-byte records through that CPU's console ring (`kernel/printk/peer_console.tkb`); the count is what the ring took, and a write that finds it full waits on its own CPU's tick and runs again |
...
| 66 | writev | Partial | fd 1/2 (UART) only -- same scoping and same fd-kind gap as `readv` above. Segments go through the same chunked, sleeping terminal path as `write`; a short segment ends the call with the count so far |
| 71 | sendfile | Implemented | fd 3 (ext2 file) -> fd 1/2 (UART) only, offset must be 0 |
| 73 | ppoll | Partial | real pollfd array validation (`struct packed Pollfd`, `POLL_MAX`-bounded) and real per-fd readiness (UART RX pending, connected-TCP buffered data). Blocks and wakes on UART RX for the BusyBox ash shape (`nfds == 1`, fd 0, `POLLIN`, NULL timeout and sigmask), and a non-NULL relative timeout uses the same per-process generic-counter deadline as nanosleep, returning 0 when it expires with nothing ready. No multi-fd event wake exists |
...
| 101 | nanosleep | Implemented | relative sleeps use one per-process generic-counter deadline and wake on the first scheduler tick at or after it; requested nanoseconds are converted to counter ticks, while actual resolution is the 1/64-second scheduler tick |
...
| 113 | clock_gettime | Implemented | `CLOCK_REALTIME` returns the fixed epoch-ish timestamp used by BusyBox HTTPd; `CLOCK_MONOTONIC` reports the architectural generic counter as seconds/nanoseconds, and unsupported clocks return `EINVAL` |
| 115 | clock_nanosleep | Partial | `CLOCK_MONOTONIC` relative and `TIMER_ABSTIME` sleeps share nanosleep's per-process deadline; other clocks and flags return `EINVAL` |
...
| 122 | sched_setaffinity | Partial | per-process mask of online CPUs, inherited across fork and kept across execve; pid 0 is the caller and an unknown pid is `ESRCH`. The mask narrows placement within the CPUs the kernel admits the process to (#9): a mask with no online CPU, or none the process is admitted to, is `EINVAL`, and a mask that later shares no admitted CPU falls back to the admitted set. The process moves at the next scheduling decision on its current CPU rather than at once. `cpusetsize` below 8 bytes is `EINVAL`; bits past the first 64 are ignored |
| 123 | sched_getaffinity | Implemented | writes the 8-byte mask `sched_setaffinity` set, or every online CPU when none was set, and returns 8 as Linux does; `cpusetsize` below 8 or not a multiple of 8 is `EINVAL`, and an unknown pid is `ESRCH` |
| 129 | kill | Partial | signal 0 checks whether a pid still names a live or zombie process; SIGCHLD is queued through the real pending-signal path; SIGTERM applies its default process-termination action at the target's next syscall boundary (or immediately when self-directed). Other signals, pid 0/process groups, and broadcast forms are rejected |
| 134 | rt_sigaction | Partial | `signum` validated (`EINVAL` for 0/`SIGKILL`/`SIGSTOP`); non-null `oldact` is validated and zero-filled via the user-memory boundary. Installed asynchronous handlers are not invoked; blocked SIGCHLD is consumed synchronously through `rt_sigtimedwait` |
