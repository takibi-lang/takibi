...
This file records the state of the kernel as of 2026-08-23. When a limit or its exhaustion behavior changes, update this
...
| ASIDs | `ASID_MAX` (`kernel/arch/arm64/mm/asid.tkb`) | 255 | Global | `asid_alloc()` / `asid_free()`, SlotMap-backed | **The one fixed ceiling left on the process-creation path.** It was 16 because it matched `KERNEL_PROCESS_MAX`, enforced by a `static_assert`; with no process table size to match, what is left is the hardware's number -- 255 is an 8-bit ASID field minus the never-assigned 0, the floor `asid.tkb` already assumed. A real `ID_AA64MMFR0_EL1.ASIDBits` query would raise it to 65535 where 16-bit ASIDs are implemented, and recycling an ASID from a live address space (Linux's rollover) is what removing the ceiling looks like; neither is implemented. `AsidAllocResult::Full` surfaces as `AddressSpaceEnsureResult::AsidExhausted` through `scheduled_process_alloc`'s diagnostic log. Boundary-tested by `asid_pool_probe`. |
...
| Shared-object reference count ceiling | `SHARED_OBJECT_MAX_REFS` (`kernel/kernel/fd_table.tkb`) | 4096, and the only number in this file that is PICKED rather than derived. It was `KERNEL_PROCESS_MAX * PROCESS_FD_MAX` (256) -- an inventory of every entry that could hold a reference at all, so no reference count could reach it. With no process ceiling that product is unbounded, so what #392 broke is the derivation, not the check. **The check is not a capacity and costs no storage**: `MAX_REFS` sizes nothing (the refcount is one `usize` per slot regardless), it is a single comparison in `refcount_pool_retain`, and what it catches is a reference LEAK long before memory would. The derivation is recoverable without changing what a refcount is: every holder is an entry in a pooled `ProcessFdContext`, so `live fd contexts x PROCESS_FD_MAX` is the exact bound, computable at each retain. That needs `refcount_slotmap.tkb`'s signature to take the bound per call instead of as a const-generic parameter | Global | Passed to `RefcountSlotMap`'s ref-take path (`unified_fd_install`/`unified_fd_duplicate`/`unified_fd_clone`) | `refcount_pool_retain()`'s own `RefcountRetainResult::Overflow` was being discarded into the same `Invalid` as a bad fd argument by `unified_fd_install`/`unified_fd_duplicate`; both now return `UnifiedFdInstallResult{Invalid;RefOverflow;Ok}`, and `fcntl(F_DUPFD)` maps `RefOverflow` to `LINUX_EMFILE` with a log line. **Auditing this surfaced a real correctness bug, not just a diagnosability gap:** `unified_fd_clone()` (fork's fd-table copy) had no rollback on a mid-loop `Overflow`/failure -- fds already retained before the failure point leaked their incremented refcounts forever, and the failing fd itself was left half-copied. Fixed via a new `unified_fd_clone_rollback()` helper. `unified_object_ref_ceiling_probe` boundary-tests the ceiling itself directly (`SHARED_OBJECT_MAX_REFS` real `refcount_pool_retain` calls on one allocated object, not a simulated shortcut -- cheap enough to just do for real, unlike `PAGE_MAP_REF_MAX`'s 4.2M). |
...
Five resources have no entry here because they no longer have a ceiling to
