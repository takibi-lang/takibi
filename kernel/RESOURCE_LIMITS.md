...
| Physical pages | **no runtime capacity constant** (`kernel/mm/page.tkb`) | all page-aligned normalized DTB memory at or above `usable_ram_start`, less the live DTB and allocator metadata | Global | `page_alloc()` / `page_free()`; a small fixed bootstrap freelist is replaced after DTB validation by runtime-sized metadata and extent descriptors | `PageAllocResult::OutOfMemory`, explicit, well-named variant. `page_alloc_contiguous` additionally refuses a logical run that crosses a physical extent boundary. |
