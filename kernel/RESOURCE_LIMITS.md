...
exists (memory, descriptor slots, address spaces). Physical-page capacity is
...
| Physical pages | **no runtime capacity constant** (`kernel/mm/page.tkb`) | every page in normalized DTB extents at or above `usable_ram_start`; pages occupied by the live DTB, bootstrap tables, or allocator metadata are retained in the inventory but are not free | Global | `page_alloc()` / `page_free()`; a small fixed bootstrap freelist is replaced after DTB validation by runtime-sized metadata and extent descriptors | `PageAllocResult::OutOfMemory`, explicit, well-named variant. Applying the DTB is fatal unless the high end of one normalized extent can hold all runtime metadata without overlapping the live DTB or a bootstrap table. `page_alloc_contiguous` additionally refuses a logical run that crosses a physical extent boundary. |
