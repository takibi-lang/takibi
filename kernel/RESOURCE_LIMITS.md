...
| Kernel stacks (per process slot) | `KERNEL_PROCESS_STACK_SIZE` x up to `KERNEL_PROCESS_MAX` chunks, via `GrowablePool` | **3968 B/slot** = `PAGE_USABLE_BYTES`, lazily grown | Global, indexed by process slot | `growable_pool_ensure()` inside `scheduled_process_alloc()` | Returns `GrowablePoolEnsureResult{IndexOutOfRange;ChunkLimitReached;OutOfMemory;Ok}`, so "this pool's own ceiling" is distinguishable from "the global physical page allocator is out of memory" at every caller. Boundary-tested by `growable_pool_probe`. **This one is not adequate and is tracked as a bug, not a limit.** It was 4096 (a whole page) until issue #373, which is 128 bytes more than a page's payload -- `kernel/mm/page.tkb` reserves `PAGE_META_BYTES` at the end of every page -- so every kernel stack's first push landed in its own page's metadata; `growable_pool` now `static_assert`s that a chunk's payload fits in `PAGE_USABLE_BYTES`. The remaining problem is size: **measured high-water marks are 3648-3968 bytes of 3968, i.e. 92-100% consumed, on both QEMU and RPi5** (`kernel_stack_watermark_report`, issue #376), and #373 caught the overflow going past the end into a neighbouring page. `kernel_stack_guard_check` reports reaching the bottom but deliberately does not halt yet, because a normal boot trips it. For calibration, Linux's `THREAD_SIZE` is 16K on arm64 and FreeBSD's `kern.kstack_pages` defaults to 4. Sizing plus a guard page is #377, which needs multi-page allocation (#350); interrupts having their own stack is #378. |
