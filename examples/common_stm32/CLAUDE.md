...
material for the `examples/common_stm32/` HAL -- Claude Code loads it
automatically whenever a file under this directory is read. It was split
out of the project root's `CLAUDE.md` to keep that file under Claude
Code's context budget (the same reasoning already applied to `HISTORY.md`,
see root CLAUDE.md's "Important Design Notes"). Read root `CLAUDE.md`
first for the project's overall goals, build commands, and process rules
